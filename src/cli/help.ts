...
export const SUBCOMMANDS: readonly SubcommandSpec[] = [
  ...
  },
  {
    ...
    aliases: [],
    args: "[shell]",
    summary: "add grove's two eval lines to your shell's rc file",
    description: [
      "Detects the running shell from $SHELL when [shell] is omitted, and",
      "appends the lines `shell-init` and `completion` print to the rc file",
      "that shell reads — ~/.zshrc, ~/.config/fish/config.fish, or for bash",
      "whichever of ~/.bashrc, ~/.bash_profile, ~/.profile already exists.",
      "",
      "Safe to run more than once: a line already there, in any spelling, is",
      "left alone — and one that is not there yet is added, which is what makes",
      "this worth running again after an upgrade.",
    ],
    flags: [],
  },
  {
  ...
  },
  {
    ...
    aliases: [],
    args: "<shell>",
    summary: "print the tab-completion script for a shell",
    description: [
      "`grove install` writes the line that loads this, so most people never",
      "type it. By hand it is one more eval beside `shell-init`'s:",
      "",
      '  eval "$(grove completion zsh)"      # zsh, bash, or fish',
      "",
      "It completes the commands, their flags, the worktrees that exist —",
      "`grove cd feat/<TAB>` — and, for `add`, the branches that do not have",
      "one yet. The script asks for those two lists as they are needed, by",
      "running `grove completion targets` and `grove completion branches`,",
      "which print one name per line and say nothing at all outside a",
      "repository: a completion is no place to report an error.",
    ],
    flags: [],
  },
  {
